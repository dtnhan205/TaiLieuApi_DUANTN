const orderEndpoints = [
      ...
      example: [
        {
          ...
          shippingStatus: 'pending', // Sửa từ 'failed' thành 'pending' cho phù hợp
          returnStatus: 'none',
  ...
  },
  {
    ...
    parameters: [
      ...
      },
      {
        name: 'shippingStatus',
        type: 'string',
        description: 'Trạng thái vận chuyển mới (`pending`, `in_transit`, `delivered`, `returned`, `cancelled`, `failed`)',
        required: false,
        in: 'body',
      },
      {
    ...
    requestExample: {
      headers: { Authorization: 'Bearer <token>' },
      body: {
        shippingStatus: 'delivered', // Sửa từ 'failed' thành 'delivered' cho phù hợp
        paymentStatus: 'completed',
        totalPrice: 250000,
        shippingAddress: {
  ...
  },
  {
    ...
    fullDescription: 'Gửi yêu cầu hoàn hàng cho một đơn hàng dựa trên orderId, chỉ cho phép khi đơn hàng ở trạng thái `delivered` và trong vòng 3-4 ngày kể từ khi đặt hàng. Yêu cầu lý do hoàn hàng và có thể đính kèm ảnh (tối đa 5 ảnh) hoặc video (tối đa 1 video) qua `multipart/form-data`. Chỉ người dùng sở hữu đơn hàng có thể gửi yêu cầu. Gửi email thông báo tới người dùng. Yêu cầu xác thực thông qua token JWT.',
    auth: {
      required: true,
      header: 'Authorization: Bearer <token>',
      description: 'Token JWT của người dùng được yêu cầu trong header. Token được cấp sau khi đăng nhập qua endpoint `/api/users/login`.',
    ...
    errorResponses: [
      ...
      { status: 400, description: 'Chỉ hỗ trợ file ảnh (jpg, jpeg, png, gif, webp, svg) hoặc video (mp4, mpeg, mov, webm) cho images/orderVideo' },
      { status: 400, description: 'Kích thước file vượt quá 100MB hoặc số lượng file vượt quá giới hạn (5 ảnh, 1 video)' },
      { status: 401, description: 'Không có token hoặc token không hợp lệ' },

const userEndpoints = [
  ...
  {
    method: 'PUT',
    path: '/api/users/temporary-addresses/:id',
    description: 'Cập nhật địa chỉ tạm thời',
    fullDescription:
      'Cập nhật địa chỉ tạm thời (temporaryAddress1) của người dùng. Nếu temporaryAddress1 đã có giá trị, giá trị cũ sẽ được đẩy sang temporaryAddress2, và temporaryAddress1 sẽ được cập nhật bằng giá trị mới. Chỉ chính người dùng hoặc admin được phép thực hiện. Yêu cầu token JWT hợp lệ hoặc xác thực Google.',
    auth: {
      required: true,
      header: 'Authorization: Bearer <token>',
      description: 'Yêu cầu token JWT hoặc xác thực Google của người dùng hoặc admin trong header Authorization.',
    ...
    parameters: [
      { name: 'id', type: 'string', description: 'ObjectId của người dùng', required: true, in: 'path' },
      { name: 'temporaryAddress1.addressLine', type: 'string', description: 'Địa chỉ chi tiết mới (tùy chọn)', required: false, in: 'body' },
      { name: 'temporaryAddress1.ward', type: 'string', description: 'Phường/xã mới (tùy chọn)', required: false, in: 'body' },
      { name: 'temporaryAddress1.district', type: 'string', description: 'Quận/huyện mới (tùy chọn)', required: false, in: 'body' },
      { name: 'temporaryAddress1.cityOrProvince', type: 'string', description: 'Thành phố/tỉnh mới (tùy chọn)', required: false, in: 'body' },
    ],
    requestExample: {
      headers: { Authorization: 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...' },
      params: { id: '60d5f8e9b1a2b4f8e8f9e2b0' },
    ...
    response: {
      status: 200,
      description: 'Cập nhật địa chỉ tạm thời thành công',
      example: {
        message: 'Cập nhật địa chỉ tạm thời thành công',
        user: {
          _id: '60d5f8e9b1a2b4f8e8f9e2b0',
          username: 'john_doe',
          phone: '0987654321',
    ...
    errorResponses: [
      { status: 400, description: 'Dữ liệu không hợp lệ hoặc id không đúng định dạng', example: { message: 'Dữ liệu không hợp lệ', errors: 'ID không hợp lệ' } },
      { status: 401, description: 'Không có token, token không hợp lệ, hoặc đã hết hạn', example: { message: 'Token không hợp lệ' } },
      { status: 403, description: 'Người dùng không có quyền cập nhật địa chỉ này', example: { message: 'Bạn không có quyền cập nhật người dùng này' } },
      { status: 404, description: 'Không tìm thấy người dùng với id cung cấp', example: { message: 'Không tìm thấy người dùng' } },
      { status: 500, description: 'Lỗi server, có thể do kết nối database', example: { message: 'Lỗi server', error: 'Database connection failed' } },
    ],
  },
